...
const News = () => {
  ...
  return (
    <div className='bg-cover md:pb-20 sm:pb-10 sm:w-full bg-blue-100' style={{backgroundImage: `url(${newsHeader})`}}>
      <header className="w-[90%] mx-auto">
        <nav className="flex justify-between items-center py-5">
            ...
            >
              ...
              {/* Desktop Menu */}
              <ul className={`hidden md:flex font-bold md:gap-12 items-center ${isMobileMenuOpen ? 'hidden' : 'block'}`}>
                        ...
                        <div className='flex items-center gap-1 relative'>
                            ...
                            {item.dropdownItems && (
                                <button
                                    ...
                                    className="p-1 hover:text-light-blue transition-colors duratio-300"
                                ...
                                {item.dropdownItems.map((dropdownItem, dropIndex) => (
                                    <li key={dropIndex}>
                                        <Link
                                            ...
                                            className="block px-4 py-2 text-gray-700 hvoer:bg-gray-50 hover:text-light-blue transition-colors duration-300"
                                        >
                                            {dropdownItem.label}
                                        </Link>
                                    </li>
                        ...
                        {item.dropdownItems ? (
                            <>
                                ...
                                >
                                    <ChevronDown 
                                        size={24}
                                        className={`transform transition-transgrom duration-300 ${openDropdowns[index] ? 'rotate-180' : ''}`}
                                    />
                                </button>
                            </>
                        ) : (
      ...
      <section className='flex sm:flex-col items-center md:pt-20 sm:pt-10 w-[90%] mx-auto'>
        <div className='flex md:flex-row sm:flex-col gap-4 justify-between md:w-[86%] sm:w-full'>
            <div className='md:w-[70%] sm:w-full'>
                <div className='md:w-[64%] sm:w-[70%] bg-white py-1 mb-2'></div>
                <h2 className='md:text-6xl sm:text-3xl text-white font-bold'>LATEST NEWS</h2>
            </div>

            <div className='md:w-[40%] sm:w-full'>
                <p className='text-lg font-normal text-white'>Keep up-to-date with all our latest news and views. To find out more see all the latest news below.</p>
            </div>
        </div>
      </section>
    </div>
